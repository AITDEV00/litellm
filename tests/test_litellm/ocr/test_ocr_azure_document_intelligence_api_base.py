...
class TestDocIntelligenceApiBaseResolution:
    def test_doc_intelligence_selects_di_provider_config(self, monkeypatch):
        """The doc-intelligence route must select the Document Intelligence
        provider config (which reads AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT), not
        the generic azure_ai one."""
        ...
        config = get_azure_ai_ocr_config("azure_ai/doc-intelligence/prebuilt-layout")
        from litellm.llms.azure_ai.ocr.document_intelligence.transformation import (
            AzureDocumentIntelligenceOCRConfig,
        )

        assert isinstance(config, AzureDocumentIntelligenceOCRConfig)
