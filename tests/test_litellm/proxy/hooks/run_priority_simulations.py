"""
Priority behaviour simulation script.
...
saturation_threshold=0.80) and prints a table of results for each scenario.
...
def setup_handler(saturation_override: float | None = None):
...
async def run_scenario(
    ...
    """
    Run a single traffic scenario.

    Args:
        name: Short scenario name
        description: Human-readable description
        traffic: {"prior1": N, "prior2": M, "prior3": K} requests per priority
        saturation_override: If set, mock saturation to this value (0.0-1.0).
                             If None, use real saturation (starts at 0, grows with traffic).

    Returns:
        Dict with results
    """
    handler, dual_cache = setup_handler(saturation_override)
    ...
    if saturation_override is not None:
        with patch.object(handler, "_check_model_saturation", return_value=saturation_override):
    ...
    return {
        ...
        "saturation_mode": (
            f"mocked={saturation_override:.0%}"
            if saturation_override is not None
            else "real (grows with traffic)"
        ),
...
def print_result_table(r: dict):
    ...
    print(f"  {'-' * 75}")
    print(f"  {'TOTAL':<12} {total_sent:>6} {total_s:>8} {total_b:>8} "
          f"{f'{total_s / total_sent * 100:.1f}%' if total_sent > 0 else 'N/A':>10} "
          f"{'100%':>8} {MODEL_RPM:>6} rpm")
    print(f"  Model capacity: {MODEL_RPM} RPM")
    print(f"  Over-capacity: {'YES' if total_s > MODEL_RPM else 'NO'} "
          f"(allowed {total_s} vs cap {MODEL_RPM})")


SCENARIOS = [
    {
        "name": "S1: Low Traffic (30 per priority)",
        "description": "90 total requests, well below 180 RPM capacity. All should succeed (generous mode).",
        "traffic": {"prior1": 30, "prior2": 30, "prior3": 30},
        "saturation_override": None,
    },
    {
        "name": "S2: At Capacity (60 per priority)",
        "description": "180 total requests, exactly at model RPM. Saturation should grow during execution.",
        "traffic": {"prior1": 60, "prior2": 60, "prior3": 60},
        "saturation_override": None,
    },
    {
        "name": "S3: Over Capacity, No Saturation Mock (200 each)",
        "description": "600 total requests. Saturation grows naturally from 0 as requests are processed.",
        "traffic": {"prior1": 200, "prior2": 200, "prior3": 200},
        "saturation_override": None,
    },
    {
        "name": "S4: Saturated (mock=95%), 200 each",
        "description": "600 total requests, saturation mocked at 95% (above 80% threshold). Strict mode throughout.",
        "traffic": {"prior1": 200, "prior2": 200, "prior3": 200},
        "saturation_override": 0.95,
    },
    {
        "name": "S5: Saturated (mock=80%), 200 each",
        "description": "600 total, saturation at exactly 80% threshold. Priority enforcement kicks in (>= comparison).",
        "traffic": {"prior1": 200, "prior2": 200, "prior3": 200},
        "saturation_override": 0.80,
    },
    {
        "name": "S6: Saturated (mock=79%), 200 each",
        "description": "600 total, saturation just below 80% threshold. Generous mode (no priority enforcement).",
        "traffic": {"prior1": 200, "prior2": 200, "prior3": 200},
        "saturation_override": 0.79,
    },
    {
        "name": "S7: prior1 Heavy (500/50/50), Saturated",
        "description": "prior1 sends 10x the traffic of others. Strict mode. prior1 capped at 90 RPM.",
        "traffic": {"prior1": 500, "prior2": 50, "prior3": 50},
        "saturation_override": 0.95,
    },
    {
        "name": "S8: prior3 Heavy (50/50/500), Saturated",
        "description": "prior3 sends 10x the traffic. Strict mode. prior3 capped at 36 RPM, prior1/prior2 unaffected.",
        "traffic": {"prior1": 50, "prior2": 50, "prior3": 500},
        "saturation_override": 0.95,
    },
    {
        "name": "S9: prior1 Only (500/0/0), Saturated",
    ...
    {
        "name": "S10: Burst (1000/1000/1000), Saturated",
        "description": "3000 total requests, all priorities equal. Strict mode. Each capped at its reservation.",
...
async def main():
    print("=" * 90)
    print("  PRIORITY BEHAVIOUR SIMULATION")
    print(f"  Config: prior1=0.50 ({PRIOR1_RPM} rpm), prior2=0.30 ({PRIOR2_RPM} rpm), "
          f"prior3=0.20 ({PRIOR3_RPM} rpm)")
    ...
    for scenario in SCENARIOS:
        r = await run_scenario(
            ...
            saturation_override=scenario.get("saturation_override"),
        )
        results.append(r)
    ...
    print(f"\n{'=' * 90}")
    print("  Done. All results above are from real DualCache-backed counter state.")
    print("=" * 90)
